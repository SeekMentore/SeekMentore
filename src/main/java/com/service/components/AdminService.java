	...
	public Map<String, Object> takeActionOnRegisteredTutors (
		...
		switch(button) {
			...
			case BUTTON_ACTION_REVERIFY : {
				query.append("IS_AUTHENTICATION_VERIFIED = 'Y', WHO_VERIFIED = ?, VERIFICATION_DATE = SYSDATE(), VERIFICATION_REMARKS = ?, RECORD_LAST_UPDATED = SYSDATE() WHERE TENTATIVE_TUTOR_ID = ?");
				params = new Object[3];
				params[0] = user.getUserId();
				params[1] = remarks;
				params[2] = tentativeTutorId;
			...
			case BUTTON_ACTION_FAILVERIFY : {
				query.append("IS_AUTHENTICATION_VERIFIED = 'N', WHO_VERIFIED = ?, VERIFICATION_DATE = SYSDATE(), VERIFICATION_REMARKS = ?, RECORD_LAST_UPDATED = SYSDATE() WHERE TENTATIVE_TUTOR_ID = ?");
				params = new Object[3];
				params[0] = user.getUserId();
				params[1] = remarks;
				params[2] = tentativeTutorId;
