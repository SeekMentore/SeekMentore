...
public class PublicAccessRestService extends AbstractRestWebservice implements RestMethodConstants, PublicAccessConstants {
	...
	public String becomeTutor (
		...
		if (this.securityPassed) {
			return JSONUtils.convertObjToJSONString(getPublicAccessService().submitApplication(application), RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		} else {
	...
	public String getDropdownListDataBecomeTutor (
		...
		doSecurity(request);
		if (this.securityPassed) {
			return JSONUtils.convertObjToJSONString(getPublicAccessService().getDropdownListData(PAGE_REFERENCE_TUTOR_REGISTRATION), RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		} else {
			return JSONUtils.convertObjToJSONString(securityFailureResponse, RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		}
	}
	...
	public String getDropdownListDataFindTutor (
		...
		doSecurity(request);
		if (this.securityPassed) {
			return JSONUtils.convertObjToJSONString(getPublicAccessService().getDropdownListData(PAGE_REFERENCE_TUTOR_ENQUIRY), RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		} else {
			return JSONUtils.convertObjToJSONString(securityFailureResponse, RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		}
	}
	...
	public String getDropdownListDataSubscribeWithUs (
		...
		doSecurity(request);
		if (this.securityPassed) {
			return JSONUtils.convertObjToJSONString(getPublicAccessService().getDropdownListData(PAGE_REFERENCE_SUBSCRIBE_WITH_US), RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		} else {
			return JSONUtils.convertObjToJSONString(securityFailureResponse, RESPONSE_MAP_ATTRIBUTE_RESPONSE_NAME);
		}
	}
	...
	private void handleBecomeTutorSecurity() throws Exception {
		...
		if (!this.securityPassed) {
			final ErrorPacket errorPacket = new ErrorPacket(new Timestamp(new Date().getTime()), 
					...
					this.securityFailureResponse.get(RESPONSE_MAP_ATTRIBUTE_MESSAGE) + LINE_BREAK + becomeTutorApplication.toString());
	...
	private void handleFindTutorSecurity() throws Exception {
		...
		if (!this.securityPassed) {
			final ErrorPacket errorPacket = new ErrorPacket(new Timestamp(new Date().getTime()), 
					this.methodName + LINE_BREAK + getActiveUserIdAndTypeForPrinting(request), 
					this.securityFailureResponse.get(RESPONSE_MAP_ATTRIBUTE_MESSAGE) + LINE_BREAK + findTutorApplication.toString());
			getCommonsService().feedErrorRecord(errorPacket);
		}
	}
	...
	private void handleSubscribeWithUsSecurity() throws Exception {
		...
		if (!this.securityPassed) {
			final ErrorPacket errorPacket = new ErrorPacket(new Timestamp(new Date().getTime()), 
					this.methodName + LINE_BREAK + getActiveUserIdAndTypeForPrinting(request), 
					this.securityFailureResponse.get(RESPONSE_MAP_ATTRIBUTE_MESSAGE) + LINE_BREAK + subscribeWithUsApplication.toString());
			getCommonsService().feedErrorRecord(errorPacket);
		}
	}
	...
	private void handleSubmitQuerySecurity() throws Exception {
		...
		if (!this.securityPassed) {
			final ErrorPacket errorPacket = new ErrorPacket(new Timestamp(new Date().getTime()), 
					this.methodName + LINE_BREAK + getActiveUserIdAndTypeForPrinting(request), 
					this.securityFailureResponse.get(RESPONSE_MAP_ATTRIBUTE_MESSAGE) + LINE_BREAK + submitQueryApplication.toString());
			getCommonsService().feedErrorRecord(errorPacket);
		}
	}
}
