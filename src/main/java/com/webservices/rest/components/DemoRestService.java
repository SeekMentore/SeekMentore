...
public class DemoRestService extends AbstractRestWebservice implements RestMethodConstants, EnquiryConstants {
	...
	public String displayPendingDemos (
		...
		if (this.securityPassed) {
			return convertObjToJSONString(getDemoService().displayCustomerWithEnquiries(REST_METHOD_NAME_DISPLAY_CUSTOMER_WITH_PENDING_ENQUIRIES, LINE_BREAK), REST_MESSAGE_JSON_RESPONSE_NAME);
		} else {
	...
	public String displaySuccessfullDemos (
		...
		doSecurity(request);
		if (this.securityPassed) {
			return convertObjToJSONString(getDemoService().displayCustomerWithEnquiries(REST_METHOD_NAME_DISPLAY_CUSTOMER_WITH_MAPPED_ENQUIRIES, LINE_BREAK), REST_MESSAGE_JSON_RESPONSE_NAME);
		} else {
			return convertObjToJSONString(securityFailureResponse, REST_MESSAGE_JSON_RESPONSE_NAME);
		}
	}
	...
	public String displayFailedDemos (
		...
		doSecurity(request);
		if (this.securityPassed) {
			return convertObjToJSONString(getDemoService().displayCustomerWithEnquiries(REST_METHOD_NAME_DISPLAY_CUSTOMER_WITH_ABANDONED_ENQUIRIES, LINE_BREAK), REST_MESSAGE_JSON_RESPONSE_NAME);
		} else {
			return convertObjToJSONString(securityFailureResponse, REST_MESSAGE_JSON_RESPONSE_NAME);
		}
	}
